...
def get_action_index(readout_t, epsilon, t):
    ...
    action_index = 0
    if(t < OBSERVE or np.random.binomial(1,epsilon)):
        action_index = np.random.randint(np.shape(readout_t)[0])
    else:
...
def scale_down_epsilon(epsilon, t):
    ...
    if( t > OBSERVE and epsilon > FINAL_EPSILON ):
        eps = epsilon - (INITIAL_EPSILON -  FINAL_EPSILON)/EXPLORE
    else:
        eps = epsilon
    return eps
